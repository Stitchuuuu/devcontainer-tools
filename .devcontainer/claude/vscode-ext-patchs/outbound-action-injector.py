...
MARKER_PENDING = 'notify-queue-outbound-perm-log-v1'
MARKER_SETTLE = 'notify-queue-outbound-perm-settle-v1'
    ...
    'if(!this._outboundStarted){'
      ...
      'try{'
        ...
        'if(wf){'
          ...
          'let pos=0;try{pos=fs.statSync(file).size;}catch(_){}'
          'let carry="";'
          'this._outboundTimer=setInterval(()=>{'
            'let sz;try{sz=fs.statSync(file).size;}catch(_){return;}'
            'if(sz<pos){pos=0;carry="";}'
            'if(sz<=pos)return;'
            'const buf=Buffer.alloc(sz-pos);let fd;'
            'try{fd=fs.openSync(file,"r");fs.readSync(fd,buf,0,buf.length,pos);}catch(_){return;}'
            ...
            'for(const line of lines){'
              ...
              'try{'
                ...
                'if(cmd.cmd==="tool_permission_response"){'
                  ...
                  'if(!p){this.output.warn(`[outbound-inject] no panel for sid=${cmd.sessionId}`);continue;}'
                  ...
                  'p.webview.postMessage({type:"from-extension",message:{type:"simulated_click",requestId:cmd.requestId,result}});'
                  'this.output.info(`[outbound-inject] posted simulated_click sid=${cmd.sessionId} rid=${cmd.requestId} behavior=${cmd.behavior}`);'
                '}else{'
                  'this.output.warn(`[outbound-inject] unknown cmd: ${cmd.cmd}`);'
                '}'
              '}catch(e){this.output.warn(`[outbound-inject] parse failed: ${e?.message??e}`);}'
            '}'
          '},200);'
          'this.output.info(`[outbound-inject] watcher started on ${file}`);'
        '}'
        ...
        'try{'
          f'if({req_var}?.type==="tool_permission_request"){{'
            ...
            'if(wf){'
              ...
              f'const rec={{ts:new Date().toISOString(),sessionId:{sid_var},requestId:{rid_var},toolName:{req_var}.toolName,inputs:{req_var}.inputs,settled:false}};'
        ...
        'try{'
          f'if({req_var}?.type==="tool_permission_request"){{'
            ...
            'if(wf){'
              'const path=require("path"),fs=require("fs");'
              'const file=path.join(wf,".devcontainer","logs","claude-code-vscode-ext-pending-perms.jsonl");'
              f'const rec={{ts:new Date().toISOString(),sessionId:{sid_var},requestId:{rid_var},settled:true,outcome:{resolve_arg}?.result?.behavior??"unknown"}};'
              'fs.promises.appendFile(file,JSON.stringify(rec)+"\\n").catch(()=>{});'
            '}'
          '}'
        '}catch(_){};'
