...
| Routing toggle | `.env` (3 URL lines value-discriminated : `http://ollama.internal:11434`, `http://claude-bridge:9223`, `http://claude-bridge.local:9223`) + `ANTHROPIC_AUTH_TOKEN`, `CLAUDE_CONFIG_DIR`) | 3 modes (`local`, `local-proxy`, `cloud`) driven by `host-helpers/claude-switch` ; sed targets the exact URL value, not just the var name, so the inactive lines stay as data. `local-proxy` auto-starts the sidecar via the same logic as `host-helpers/claude-bridge up`. Per-profile prompt files `CLAUDE-local-<name>-dev.md` are a planned convention (current files : `CLAUDE-local-dev.md` only) — the discoverable `/tune-claude-local` skill that lands per-profile content lives in [`plans/tune-claude-local-skill/`](../../plans/tune-claude-local-skill/ROLLOUT.md), deferred for v1 |
...
| Manual tuning harness | `tests/diag-bridge-translation.sh` + `tests/tweak-claude-md-for-local.sh` | Cloud-as-oracle measurement + idempotent variant applier between `<!-- 1B-LIGHT-MODEL-DIRECTIVES-START/END -->` markers of `CLAUDE-local-dev.md`. Workflow documented in [ollama-local.md § Tuning](ollama-local.md#tuning-the-local-prompt-for-your-hardware-current-ad-hoc-workflow). **Future** : the `/tune-claude-local` skill planned in [`plans/tune-claude-local-skill/`](../../plans/tune-claude-local-skill/ROLLOUT.md) will industrialize this loop ; deferred for v1 |
...
- Run [UniClaudeProxy](https://github.com/vibheksoni/UniClaudeProxy)
  ...
  thinking shipped) — rollout plan : [plans/uniclaudeproxy-integration/ROLLOUT.md](../../plans/uniclaudeproxy-integration/ROLLOUT.md).
...
| Editing `phases/` directory | Removed in A5 cleanup; superseded by `plans/devcontainer-v2/phase3-rollout/sessions/` | Add new sessions to the rollout instead |
