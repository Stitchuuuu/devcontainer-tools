---
description: Review project dependencies. Runs the deterministic extract-auto-dependencies (npm) then layers AI analysis on top — flag suspicious deps, postinstall scrutiny, POST candidates, suggest /prepare-research for scope outside the baseline.
argument-hint: "[--no-extract] [--ecosystem npm|composer|python|cargo|go]"
---
...
1. **Layer 1 — deterministic extraction** : invoke
   ...
   (F2 implements npm ; other ecosystems fall through to AI handling).
...
ecosystems). F2 implementation : only npm has a full extractor. For
composer / python / cargo / go, `extract-auto-dependencies` exits
silently and the skill takes over (Step 2 + 3 do everything).
...
For composer / python / cargo / go, manually invoke the corresponding
parser (the F1 logic, now consolidated into the skill body) :

- Detect manifest : `composer.json`, `pyproject.toml`, `requirements.txt`,
  `Cargo.toml`, `go.mod` under `/workspace` (depth-bumped : 3, 5, 8, 10 ;
  excludes `node_modules`, `vendor`, `.git`, `research-bundles`).
- Enumerate deps from manifest + lock file. Emit per-dep paths on the
  matching registry (packagist → `/p2/<vendor>/<pkg>.json*` ;
  pypi → `/simple/<pkg>/*` ; crates → `/api/v1/crates/<name>*` ;
  go.mod → `<host>/<owner>/*`).
...
- **No automatic edits** to `firewall/domains.txt` (the Claude-only
  ...
  `extract-auto-dependencies` for npm, by the skill for other
  ecosystems with user confirmation).
- **No network** — all parsers offline.
...
- Writes : `firewall/domains.d/npm.txt` (via extract-auto), maybe
  `firewall/domains.d/<other-eco>.txt` (skill with confirmation),
  `.devcontainer/scan-deps/<ts>.md` (audit), `.last-scan.json` (sentinel).
