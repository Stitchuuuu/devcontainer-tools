...
pub fn dispatch_send(notif: &Notification, aumid: &str) -> Result<(), WindowsError> {
    let start = std::time::Instant::now();
    let scenario = resolve_scenario(notif.priority);
    debug!(target: "notif::send", ?scenario, "resolved scenario");

    let xml = build_toast_xml(notif, scenario);
    ...
    if let Some(id) = &notif.id {
        let htag = HSTRING::from(id.as_str());
        toast
            .SetTag(&htag)
    ...
    let id_for_log = notif.id.as_deref().unwrap_or("<none>");
    info!(
        target: "notif::send",
...
fn build_toast_xml(notif: &Notification, scenario: Option<&str>) -> String {
    ...
    let scenario_attr = scenario
        ...
        .unwrap_or_default();
    format!(
        "<toast{scenario_attr}><visual><binding template=\"ToastGeneric\"><text>{title}</text>{subtitle_line}<text>{body}</text></binding></visual><audio silent=\"true\"/></toast>"
    )
}
...
mod tests {
    ...
    fn basic_toast_shape() {
        ...
        let xml = build_toast_xml(&n, None);
    ...
    fn scenario_attribute_when_set() {
        ...
        let xml = build_toast_xml(&n, Some("urgent"));
