...
//!   `CFBundleIdentifier`.
//! - **Tier 1** — identity spoof via [`nsun`], dispatching through
//!   `NSUserNotificationCenter` after swizzling `[NSBundle bundleIdentifier]`
//!   to the target app's identifier — no local `.app` materialized.
//! - **Tier 2** — user-registered custom senders via `notif register`.
//! - **Tier 3** — raw `--macos-*` overrides applied inside [`dispatch::inner`]
//!   after the portable fields (native wins with an `info:` log).
//!
