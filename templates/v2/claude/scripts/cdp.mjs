...
const ALLOW_FILE_BAKED = '/etc/devcontainer-firewall/direct-tcp-allow.txt'
const ALLOW_FILE_SOURCE = resolve(SCRIPT_DIR, '../../firewall/direct-tcp-allow.txt')
 ...
 * from direct-tcp-allow.txt, and a Chromium that simply is not running looks
...
function unreachableHint(ep, err) {
	...
	if (baked === false && source === true) {
		return `${head}
  → host:${ep.port} is allowed in .devcontainer/firewall/direct-tcp-allow.txt but NOT in the
  ...
  → the devcontainer firewall does not allow host:${ep.port}. Add it to
    .devcontainer/firewall/direct-tcp-allow.txt, then Rebuild Container.`
	}
