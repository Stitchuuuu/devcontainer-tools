...
|---|---|---|
| **match / overkill** | current model ≥ tier primary | #1 « This session » stays recommendable (default). If overkill by ≥2 ranks (e.g. fable for tier C/D), note the wasted cost — still allowed. |
...
| `{{model_tier_cell}}` | STATUS Model cell, e.g. `B (opus-4.8)` |
...
|---|---|---|---|
| **A** | Max reasoning | `fable` → `opus-4.8` → `opus-4.7` | `opus-4.8` → `opus-4.7` → `sonnet-5` |
| **B** | Default | `opus-4.8` → `opus-4.7` → `sonnet-5` | (same) |
| **C** | Specified execution | `sonnet-5` → `opus-4.7` | (same) |
| **D** | Mechanical | `haiku-4.5` → `sonnet-5` | (same) |

Ranking (for gates) :
`fable > opus-5 ≈ opus-4.8 > opus-4.7 > sonnet-5 > haiku-4.5`.

Opus 5 and 4.8 tie at rank 4 (same price / context / class). Ladders
default to 4.8 because 5 empirically refuses more on offensive-framed
prompts. Non-sensitive sessions can substitute 5 for 4.8 in tier A/B ;
`*` locks that off.
...
When it fires : tier suffix → `A*` / `B*` / etc. ; tier A ladder swaps
to the classifier-free variant ; Opus 5 substitution off ; announce
"classifier-sensitive framing detected — classifier-free ladder locked"
in the pre-question context.
...
1. The Model line (tier gets `*` if classifier gate fired) :
   ...
   ```
   > **Model : Tier B — opus-4.8** (fallback : opus-4.7 → sonnet-5). Run /model before pasting this prompt.
   ```
...
3. For ALL tiers, the self-check gate block :
   ...
   **Model self-check (first action, before any work)** : check the model
   ...
   session's tier.
   - Match or above : proceed silently.
   - In the fallback ladder but not primary : proceed, applying the
     degraded-mode directives.
...
- `{{model_line_short}}` — one line, no /model sentence, no gate (for
  STATUS files) : `Tier B — opus-4.8 (fallback : opus-4.7 → sonnet-5)`.
...
- `{{tier_legend}}` — one line per **distinct tier used** in the file
  ...
  - **B** = opus-4.8 (fallback : opus-4.7 → sonnet-5)
  - **C** = sonnet-5 (fallback : opus-4.7)
  - `*` suffix = classifier-sensitive session ; classifier-free set locked (Opus 5 / Fable substitution off).
  ```
...
- Modes §0.5 and §0.6 : share `prompt-body`.
...
|---|---|---|
| **A** | Max reasoning | fable → opus-4.8 → opus-4.7 |
| **B** | Default | opus-4.8 → opus-4.7 → sonnet-5 |
| **C** | Specified execution | sonnet-5 → opus-4.7 |
| **D** | Mechanical | haiku-4.5 → sonnet-5 |
...
4. **§0.6 only** — update `plans/{{feature_name}}/STATUS.md` : flip the
