        ...
        async def react_event_generator():
            ...
            try:
                if route.provider_type == "gemini":
                    ...
                    raw_stream = openai_provider.send_streaming(request, route)
                    upstream = stream_openai_chat_to_anthropic(raw_stream, anthropic_model, enable_thinking=route.enable_thinking)
...
async def _handle_force_stream_non_streaming(
    ...
    try:
        if route.provider_type == "gemini":
            ...
            raw_stream = openai_provider.send_streaming(request, route)
            converter = stream_openai_chat_to_anthropic(raw_stream, anthropic_model, enable_thinking=route.enable_thinking)
    ...
    async def event_generator():
        ...
        try:
            if route.provider_type == "gemini":
                ...
                raw_stream = openai_provider.send_streaming(request, route)
                async for event in stream_openai_chat_to_anthropic(raw_stream, anthropic_model, enable_thinking=route.enable_thinking):
                    yield event
