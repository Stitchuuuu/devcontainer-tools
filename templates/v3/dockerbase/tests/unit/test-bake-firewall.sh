...
#   - new baked files present (default-mode, ports.txt, .keep)
...
test_baked_files_present() {
  local paths=(
    ...
    "templates/v2/firewall/ports.txt"
...
test_init_firewall_reads_baked() {
  ...
  assert_contains "$f" "/etc/devcontainer-firewall/ports.txt" \
    "init-firewall.sh reads ports.txt baked"
}
...
test_test_firewall_reads_baked() {
  ...
  assert_contains "$f" "/etc/devcontainer-firewall/ports.txt" \
    "test-firewall.sh reads ports.txt baked"
}
...
test_install_sh_has_migration() {
  ...
  assert_contains "$f" "default-mode ports.txt" "install.sh copies new baked files"
}
...
EOF
  ...
  local tcp_file="$DEST/firewall/ports.txt"
  ...
  assert_contains "$TMP/firewall/ports.txt" "claude-bridge:9223" "migration: tcp entry 1"
  assert_contains "$TMP/firewall/ports.txt" "host:11434"          "migration: tcp entry 2"
