...
# reload-firewall, and asserts the zero-downtime contract PLUS the sibling
...
#
# Success: exit 0 (VALIDATION PASSED marker), "__END__" sentinel, 15 "✔"
...
echo
echo "═══ [14] Host isolation — host.docker.internal:$HOST_PORT_BLOCKED must be BLOCKED ═══"
...
# is in a private range). The ipset ACCEPT for allowed-domains never gets
...
echo
echo "═══ [15] Host opt-in — host.docker.internal:$HOST_PORT_ALLOWED must PASS (host:$HOST_PORT_ALLOWED seed) ═══"
# Same setup as [14] but this port IS seeded into direct-tcp-allow.txt as
