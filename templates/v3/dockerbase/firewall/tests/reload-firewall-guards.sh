...
# A workspace overlay with one extra host, so the dry run has something to show.
cp -a /workspace/.devcontainer/firewall "$SB/fw"
...
cp -a /workspace/.devcontainer/firewall "$SB/fw-empty"
