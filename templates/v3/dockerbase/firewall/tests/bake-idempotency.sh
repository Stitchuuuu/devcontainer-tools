...
for f in domains.txt domains.local.txt domains.d policy.d policy.local.d \
         default-mode direct-tcp-allow.txt CLAUDE.md; do
...
[ "$LOCAL_HOSTS" -gt "$HARD_HOSTS" ] && ok "opted-in $LOCAL_HOSTS hosts > hardened $HARD_HOSTS" \
                                     || ko "opted-in $LOCAL_HOSTS !> hardened $HARD_HOSTS"
...
if [ -r "$LIVE_RUN/dnsmasq-domains-base.conf" ]; then
  ...
  # direct-tcp-allow host from the live conf after compiling. Replay that on
  # the candidate, and drop the Generated-at stamp on both sides.
  tcp_hosts=$(sed 's/#.*//' "$LIVE_FW/direct-tcp-allow.txt" 2>/dev/null \
              | cut -d: -f1 | tr -d '[:space:]' | grep -v '^$' \
              | sed 's/\./\\./g' | paste -sd'|' -)
  norm() {
