#!/bin/bash
# initialize/notify-daemon.sh — sourced by initialize.sh
#
...
# happened on disk.
#
# Reads : DEVCONTAINER_DIR, PROJECT_DIR, HOST_KIND, ORIG_STDOUT_TTY (all set
# by initialize.sh), plus host env (HOME, PATH, NVM_DIR, NO_COLOR, ...).
# Writes: stdout only (teed to initialize log via the caller's redirect).
